...
function ModalJustificacao({ faltaId, onClose, onSuccess, token }: ModalJustificacaoProps) {
...
function ResumoCards({ resumo }: { resumo: FaltaResumoDto | null }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px', marginBottom: '24px' }}>
      {cards.map(c => (
        <div key={c.label} style={{ background: c.bg, border: `1px solid ${c.borda}`, borderLeft: `4px solid ${c.cor}`, borderRadius: '8px', padding: '16px 18px' }}>
          <p style={{ margin: '0 0 4px', fontSize: '10px', color: c.cor, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{c.label}</p>
          <p style={{ margin: 0, fontSize: '26px', fontWeight: 500, color: c.cor }}>{c.value}</p>
        </div>
      ))}
    </div>
  );
...
function TabelaFaltas({ faltas, token, mostrarJustificar, mostrarEliminar, mostrarValidar, onJustificar, onEliminar, onValidar }: TabelaFaltasProps) {
  ...
  if (faltas.length === 0) {
    return (
      <div style={{ padding: '40px', textAlign: 'center', color: 'var(--accent-muted, #aaa)', fontSize: '13px', fontStyle: 'italic' }}>
        Nenhuma falta encontrada para os critérios selecionados.
      </div>
    );
  }
...
function SecaoAluno({ token, role }: { token: string; role: Role }) {
  ...
  return (
    <div>
      ...
      {faltaParaJustificar && (
        <ModalJustificacao faltaId={faltaParaJustificar} token={token} onClose={() => setFaltaParaJustificar(null)} onSuccess={carregar} />
      )}
    </div>
  );
}
  ...
  const handleToggleAula = async (aulaId: string) => {
    ...
    try {
      ...
      if (resFaltas.ok) {
        ...
        setAlunosComFalta(new Set(faltas.map(f => typeof f.utilizadorId === 'object' ? f.utilizadorId?.id : f.utilizadorId ?? '').filter(Boolean)));
      }
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '4px', background: '#F5EFE6', padding: '4px', borderRadius: '8px', marginBottom: '24px', width: 'fit-content' }}>
        <button style={tabStyle(tab === 'marcar')} onClick={() => setTab('marcar')}><i className="ti ti-edit" style={{ marginRight: 6 }} />Marcar Faltas</button>
        <button style={tabStyle(tab === 'historico')} onClick={() => setTab('historico')}><i className="ti ti-list" style={{ marginRight: 6 }} />Histórico</button>
      </div>
  ...
  return (
    <div>
      ...
      {(tab === 'todas' || tab === 'pendentes') && (
        <div style={{ background: '#FFF', border: '1px solid var(--border-warm, #e5dec9)', borderRadius: '8px', overflow: 'hidden' }}>
          {/* Header com pesquisa */}
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #F5EFE6', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: '16px', flexWrap: 'wrap' }}>
            <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <input
                type="text"
                placeholder="Digite nome ou motivo…"
                value={pesquisaNome}
                onChange={e => { setPesquisaNome(e.target.value); setPagina(0); }}
                style={{ ...estiloInput, width: '240px' }}
              />
            </div>
            {ultimaAtualizacao && (
              <span style={{ fontSize: '11px', color: '#bbb', alignSelf: 'flex-end' }}>
                Atualizado às {ultimaAtualizacao.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}
              </span>
            )}
          </div>
