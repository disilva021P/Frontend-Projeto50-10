...
export default function EventosPage() {
    ...
    // ── ESTADOS DE CONTROLO VISUAL & NAV (Igual ao Marketplace) ──
    ...
    const marcarTodasComoLidas = async () => {
        ...
        try {
            await Promise.all(naoLidas.map((n) => {
                if (String(n.id).startsWith('m')) return Promise.resolve(); // pula chamada se for mock
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background font-sans text-panel-dark">
            ...
            {/* ── NAVBAR SUPERIOR INTEGRADA (Estilo Marketplace) ── */}
            <nav
