...
export default function EventosPage() {
    ...
    const handleInscrever = async (eventoId: string) => {
        try {
            ...
            setEventos(prev => prev.map(ev => ev.id === eventoId ? { ...ev, numInscritos: String(Number(ev.numInscritos) + 1) } : ev));
            carregarInscricoes();
        } catch (err: any) {
...
const handleEditar = async (e: React.FormEvent) => {
    ...
    const eventosExibidos = filtroMeus ? eventos.filter(e => meusEventosIds.includes(e.id)) : eventos;
        ...
        <main className="flex-1 overflow-y-auto p-6 text-[#2d2722] selection:bg-[#4a3f35] selection:text-white">
                        ...
                        {eventosExibidos.map((evento) => {
                            ...
                            return (
                                <div key={evento.id} className="bg-white border border-[#e6e1d6] rounded-2xl p-6 flex flex-col hover:shadow-sm transition-all duration-300">
                                    ...
                                    <p className="text-[#6b6155] text-xs leading-relaxed line-clamp-3 mb-6 min-h-[54px]">{evento.descricao}</p>
                                    <div className="flex gap-2 mt-auto pt-2 border-t border-[#f4f1ea]">
                                        <button onClick={() => setEventoSelecionado(evento)} className="flex-1 bg-[#faf9f6] border border-[#e6e1d6] text-[#2d2722] hover:bg-[#f4f1ea] py-2.5 rounded-xl text-xs font-bold transition">Ver Detalhes</button>
                                        {estaInscrito ? (
                                            <button onClick={() => handleCancelar(evento.id)} className="flex-1 bg-[#fff5f5] text-red-600 border border-red-200 hover:bg-red-600 hover:text-white py-2.5 rounded-xl text-xs font-bold transition">Cancelar</button>
                                        ) : (
                                            <button onClick={() => handleInscrever(evento.id)} className="flex-1 bg-[#4a3f35] hover:bg-[#382f27] text-white py-2.5 rounded-xl text-xs font-bold transition">Inscrever</button>
                                        )}
                                    </div>
                                </div>
                            );
            ...
            {eventoSelecionado && !ehAdministrador && (
                <div className="fixed inset-0 bg-[#2d2722]/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-[#e6e1d6] w-full max-w-lg rounded-2xl overflow-hidden shadow-xl">
                        <div className="p-8">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <div className="inline-block px-2.5 py-0.5 bg-[#f4f1ea] border border-[#e6e1d6] text-[#6b6155] rounded-full text-[9px] font-bold tracking-wider uppercase mb-2">Evento Cultural</div>
                            ...
                            {!meusEventosIds.includes(eventoSelecionado.id) ? (
                                <button onClick={() => { handleInscrever(eventoSelecionado.id); setEventoSelecionado(null); }} className="w-full bg-[#4a3f35] hover:bg-[#382f27] py-3 rounded-xl font-bold text-white text-xs transition">Confirmar Inscrição</button>
                            ) : (
            ...
            {participantesModal.aberto && ehAdministrador && (
                <div className="fixed inset-0 bg-[#2d2722]/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-[#e6e1d6] w-full max-w-md rounded-2xl overflow-hidden shadow-xl">
            ...
            {showEditModal && eventoEditando && ehAdministrador && (
                <div className="fixed inset-0 bg-[#2d2722]/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-[#e6e1d6] w-full max-w-lg rounded-2xl overflow-hidden shadow-xl">
                        ...
                        <form onSubmit={handleEditar} className="p-5 space-y-4 text-xs max-h-[70vh] overflow-y-auto">
                            <div>
            ...
            {showModal && ehAdministrador && (
                <div className="fixed inset-0 bg-[#2d2722]/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-[#e6e1d6] w-full max-w-md rounded-2xl overflow-hidden shadow-xl">
                        ...
                        <form onSubmit={handleCriar} className="p-5 space-y-3 text-xs">
                            <div>
                                <label className={labelClass}>Nome do Evento</label>
