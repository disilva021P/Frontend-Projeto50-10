...
export default function AdminEventosPage() {
    ...
    // --- NOVOS ESTADOS PARA OS PARTICIPANTES ---
    const [showParticipantesModal, setShowParticipantesModal] = useState(false);
    const [participantes, setParticipantes] = useState<any[]>([]);
    const [loadingParticipantes, setLoadingParticipantes] = useState(false);
    const [eventoSelecionadoNome, setEventoSelecionadoNome] = useState('');
    ...
    // --- NOVA FUNÇÃO: CARREGAR PARTICIPANTES DO EVENTO ---
    const handleVerParticipantes = async (eventoId: string, nomeEvento: string) => {
        try {
            setEventoSelecionadoNome(nomeEvento);
            setShowParticipantesModal(true);
            setLoadingParticipantes(true);
            setParticipantes([]);
            
            // Certifica-te de que a rota bate certo com o teu ParticipanteController do Java
    ...
    const handleEliminar = async (id: string) => {
        ...
        try {
            ...
            if (err.response && err.response.status === 403) {
                toast.error("Não tens permissões de administrador para eliminar.");
            } else {
                                                ...
                                                <tr key={ev.id} className="hover:bg-[#FFFCF8] transition-colors">
                                                    ...
                                                    <td className="p-4 text-center whitespace-nowrap">
                                                        <span className="inline-flex items-center gap-1 bg-[#f4f1ea] border border-[#e6e1d6] px-2.5 py-1 rounded-md text-xs font-medium">
                                                            <span className="text-[#2d2722]">{ev.totalInscritos ?? 0}</span>
                                                    ...
                                                    <td className="p-4 whitespace-nowrap text-right">
                                                        <div className="inline-flex gap-1.5">
                                                            {/* ADICIONADO: Evento onClick para abrir os participantes reais */}
                                                            <button 
                                                                onClick={() => handleVerParticipantes(ev.id, ev.nome)}
    ...
    {/* ─── MODAL: ALUNOS INSCRITOS ─── */}
            {showParticipantesModal && (
                <div className="fixed inset-0 bg-[#2d2722]/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
                    <div className="bg-white border border-[#e6e1d6] w-full max-w-md rounded-xl overflow-hidden shadow-xl">
                        
                        <div className="p-5 border-b border-[#e6e1d6] bg-[#FBF7F2] flex justify-between items-center">
                            <div>
                                <h2 className="text-base font-serif font-semibold text-[#2d2722]">Alunos Inscritos</h2>
                                <p className="text-[11px] text-[#8c8275] mt-0.5">{eventoSelecionadoNome}</p>
                            </div>
                            <button onClick={() => setShowParticipantesModal(false)} className="text-[#8c8275] hover:text-[#2d2722] transition-colors text-sm">✕</button>
                        </div>

                        <div className="p-5 max-h-[350px] overflow-y-auto space-y-3">
                            {loadingParticipantes ? (
                                <div className="flex justify-center py-8">
                                    <div className="w-5 h-5 border-2 border-[#2d2722] border-t-transparent rounded-full animate-spin"></div>
                                </div>
                            ) : participantes.length === 0 ? (
                                <div className="text-center py-8 text-xs text-[#8c8275] font-light">
                                    Nenhum aluno inscrito neste evento até ao momento.
                                </div>
                            ) : (
                                participantes.map((part: any, index: number) => (
                                    <div key={index} className="p-3 bg-[#FFFCF8] border border-[#e6e1d6] rounded-lg">
                                        <p className="text-xs font-medium text-[#2d2722]">{part.utilizadorNome}</p>
                                        <p className="text-[10px] text-[#8c8275]">{part.utilizadorEmail}</p>
                                        {part.cancelado ? (
                                            <span className="text-red-500 text-[10px]">Cancelado</span>
                                        ) : part.pago ? (
                                            <span className="text-green-600 text-[10px]">Pago</span>
                                        ) : (
                                            <span className="text-amber-600 text-[10px]">Pendente</span>
                                        )}
                                    </div>
                                ))
                            )}
                        ...
                        <div className="p-4 bg-[#FBF7F2] border-t border-[#e6e1d6] flex justify-end">
                            <button onClick={() => setShowParticipantesModal(false)} className="px-4 py-2 bg-[#2d2722] text-[#d4b288] rounded-md text-xs font-medium hover:bg-[#3d332a] transition-colors">
                                Fechar
                            </button>
                        </div>
                    </div>
