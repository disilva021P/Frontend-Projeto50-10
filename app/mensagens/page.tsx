...
// Tipagem específica para as notificações do sino da Navbar
interface NavbarNotificacao {
  id: string;
  titulo: string;
  mensagem: string;
  lida: boolean;
...
export default function MensagensPage() {
  ...
  // Estados da Navbar e Dropdowns
  const [showNotifPanel, setShowNotifPanel] = useState(false);
  const [showProfileMenu, setShowProfileMenu] = useState(false);
  const [unreadCount, setUnreadCount] = useState<number>(0);
  const [navbarNotificacoes, setNavbarNotificacoes] = useState<NavbarNotificacao[]>([]);
  const [drawerOpen, setDrawerOpen] = useState(false); // Estado caso uses a sidebar geral do teu app
  ...
  const marcarTodasComoLidas = async () => {
    setUnreadCount(0);
    setNavbarNotificacoes(prev => prev.map(n => ({ ...n, lida: true })));
    // Se tiveres um endpoint para marcar todas como lidas, podes chamar aqui:
    // try { await api.put("/notificacoes/ler-todas"); } catch(e) {}
  };
  ...
  // Polling para alimentar as notificações em tempo real do Sino da Navbar
  const fetchNavbarNotificacoes = useCallback(async () => {
    if (!currentUserHashId) return;
    try {
      const { data } = await api.get(`/notificacoes/me?userId=${currentUserHashId}&page=0&size=5`);
      const content = data.content || [];
      
      // Transforma o DTO do servidor no formato simples esperado pela Navbar
      const mapeadas: NavbarNotificacao[] = content.map((n: any) => ({
        id: n.id,
        titulo: n.titulo,
        mensagem: n.mensagem,
        lida: n.lida
      ...
      setNavbarNotificacoes(mapeadas);
      setUnreadCount(mapeadas.filter(n => !n.lida).length);
    } catch (err) {
      console.error("Erro ao carregar notificações na navbar", err);
    }
  }, [currentUserHashId]);

  useEffect(() => {
    fetchNavbarNotificacoes();
    const interval = setInterval(fetchNavbarNotificacoes, 30000);
    return () => clearInterval(interval);
  }, [fetchNavbarNotificacoes]);

  // Quando alguém clica numa notificação do Sino
  const clicarNotificacaoSino = async (n: NavbarNotificacao) => {
    try {
  ...
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
          ...
          --chat-sent: #503c25;        
          --chat-sent-text: #FFFCF8;
          --chat-received: #F3ECE2;    
          --chat-received-text: #2C251E;
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100dvh", overflow: "hidden", background: "var(--bg-main)" }}>
        
        {/* ── NAVBAR (Inserida e Com Dropdowns Interativos) ── */}
        <nav
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-panel-dark text-[8px] font-normal w-4 h-4 flex items-center justify-center rounded-full text-accent-gold" style={{ background: "var(--panel-dark)", color: "var(--accent-gold)" }}>
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              {showNotifPanel && (
                <div className="absolute right-0 mt-2 w-72 bg-[#FBF7F2] border border-border-warm rounded-sm shadow-xl z-50 overflow-hidden text-left">
                  ...
                  <div className="max-h-64 overflow-y-auto divide-y divide-border-warm/30">
                    {navbarNotificacoes.length === 0 ? (
                      <p className="p-6 text-center text-accent-muted text-xs font-light">
                        Sem novas notificações.
                      </p>
                    ) : (
                      navbarNotificacoes.map((n) => (
                        <div
                          key={n.id}
                          onClick={() => clicarNotificacaoSino(n)}
                          className="p-3 hover:bg-[#FFFCF8] transition-colors cursor-pointer"
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {!n.lida && (
                              <span className="w-1.5 h-1.5 rounded-full bg-accent-gold mt-1 flex-shrink-0" style={{ backgroundColor: "var(--accent-gold)" }}></span>
                            )}
                          </div>
            ...
            <div className="relative">
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-[#FBF7F2] border border-border-warm rounded-sm shadow-xl z-50 overflow-hidden py-1 text-left">
        ...
        {/* ── CONTEÚDO PRINCIPAL (OCUPA O ESPAÇO RESTANTE ABAIXO DA NAVBAR) ── */}
        <div style={{ display: "flex", flex: 1, height: "calc(100dvh - 52px)", overflow: "hidden" }}>
