...
export default function UtilizadoresPage() {
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => { if (e.key === "Escape") { setDrawerOpen(false); setDetalhe(null); setModalAberto(false); setReporTarget(null); } };
  ...
  const handleSalvarUtilizador = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        dataNascimento: form.dataNascimento, id_tipoUtilizador: form.id_tipoUtilizador,
        ...
        idTurmasIniciais: form.id_tipoUtilizador === hashesDiscobertas.ALUNO ? form.idTurmasIniciais : [],
      };
  ...
  // ── Erro 1 corrigido: renomeado de handle開設Utilizador para handleGuardarUtilizador ──
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--background)", fontFamily: "var(--font-lato)" }}>
        ...
        {/* Toasts */}
        {(successMsg || errorMsg) && (
          <div style={{ position: "fixed", top: 68, right: 24, zIndex: 110, animation: "fadeUp 0.2s ease", maxWidth: 320, padding: "12px 16px", borderRadius: 6, fontSize: 13, border: "1px solid", background: successMsg ? "#f0fdf4" : "#fef2f2", color: successMsg ? "#15803d" : "#991b1b", borderColor: successMsg ? "#bbf7d0" : "#fecaca" }}>
            {successMsg || errorMsg}
          </div>
        )}
        ...
        <nav style={{ height: 52, borderBottom: "1px solid var(--border-warm)", background: "var(--background)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", flexShrink: 0, position: "sticky", top: 0, zIndex: 40 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => setDrawerOpen(true)} style={{ width: 32, height: 32, border: "1px solid var(--border-warm)", borderRadius: 4, background: "#FFFCF8", color: "var(--panel-dark)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 12, color: "var(--accent-muted)", fontWeight: 300 }}>{userName ? `Bem-vindo, ${userName.split(" ")[0]}` : ""}</span>
            <div style={{ position: "relative" }}>
              <div onClick={() => setShowProfileMenu(!showProfileMenu)} style={{ width: 30, height: 30, borderRadius: "50%", background: "var(--panel-dark)", color: "var(--accent-gold)", fontSize: 11, letterSpacing: 1, fontFamily: "var(--font-playfair)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                {userInitials}
              </div>
              {showProfileMenu && (
                <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 192, background: "#FBF7F2", border: "1px solid var(--border-warm)", borderRadius: 4, boxShadow: "0 8px 24px rgba(44,28,10,0.12)", zIndex: 50, overflow: "hidden", padding: "4px 0" }}>
                  <div style={{ padding: "8px 12px", borderBottom: "1px solid rgba(160,133,96,0.15)", background: "#FFFCF8" }}>
                    ...
                    <p style={{ fontSize: 12, color: "var(--panel-dark)" }}>{userName || "Utilizador"}</p>
                  </div>
                  <button onClick={() => { router.push("/utilizadores/verPerfil"); setShowProfileMenu(false); }} style={{ width: "100%", textAlign: "left", padding: "8px 12px", fontSize: 12, color: "var(--panel-dark)", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={handleLogout} style={{ width: "100%", textAlign: "left", padding: "8px 12px", fontSize: 12, color: "#c0392b", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "flex", flex: 1, position: "relative", overflow: "hidden" }}>
          {drawerOpen && <div style={{ position: "absolute", inset: 0, zIndex: 10, background: "rgba(44,31,20,0.30)" }} onClick={() => setDrawerOpen(false)} />}

          {/* ── DRAWER ── */}
          <aside style={{ position: "absolute", top: 0, bottom: 0, left: 0, zIndex: 20, width: 220, background: "#503c25", boxShadow: "4px 0 24px rgba(44,28,10,0.30)", transform: drawerOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform .28s cubic-bezier(.4,0,.2,1)", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "20px", borderBottom: "1px solid rgba(245,217,168,0.12)" }}>
              <span style={{ fontFamily: "var(--font-playfair)", fontSize: 14, letterSpacing: 3, color: "#FFF8EE", display: "block" }}>entartes</span>
              <span style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: "rgba(245,217,168,0.65)", marginTop: 4, display: "block" }}>escola de dança</span>
            </div>
                ...
                <div key={section.title} style={{ marginBottom: 14 }}>
                  ...
                  {section.items.map(item => (
                    <button key={item.href} onClick={() => { router.push(item.href); setDrawerOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 20px", color: item.href === "/utilizadores" ? "var(--accent-gold)" : "rgba(255,248,238,0.75)", background: item.href === "/utilizadores" ? "rgba(255,255,255,0.08)" : "transparent", border: "none", fontSize: 13, fontWeight: 300, cursor: "pointer", textAlign: "left" }}>
                      <i className={`ti ${item.icon}`} style={{ fontSize: 15, opacity: .85 }} />
                      {item.label}
                    </button>
                  ))}
                </div>
      ...
      {/* ══ MODAL DETALHE ══ */}
      {/* Erro 2 corrigido: modais movidos para fora do modal de detalhe */}
      {detalhe && (
